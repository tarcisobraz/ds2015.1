...
class AsyncRMQConsumer(object):
    ...
    def __init__(self, amqpAddress,queueName,swift_user,swift_key,outputFolderPath,verbose):
    ...
    def retrieveSwiftObject(self,objectPath):
        fileName = objectPath.split("/")[-1]
        outputFilePath = self._outputFolderPath + "/" + fileName
        container_name = "tarciso-container"
        if (self._verbose):
                print "Downloading Object", objectPath,"..."
        obj_tuple = self._swiftConn.get_object(container_name, objectPath)
   ...
   def on_message(self, unused_channel, basic_deliver, properties, body):
        ...
        procTime = decoded_msg[0]["proc_time"]
        self.retrieveSwiftObject(filePath)
        print "Processing..."
        for i in range(procTime):
            time.sleep(1)
            print "."
        print " [x] Done"
    ...
    def getSwiftConnection(self,user,key):
        osOptions = dict()
...
def main():
    ...
    consumer = AsyncRMQConsumer(amqpAddress,queueName,swift_user,swift_key,outputFolderPath,verbose)
    consumer.getSwiftConnection(swift_user,swift_key)
